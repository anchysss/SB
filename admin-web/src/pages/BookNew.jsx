...
import { createBook, updateChapter } from '../api'
...
export default function BookNew() {
  ...
  const [form, setForm] = useState({
    ...
    category: 'Romance', tags: '', cover_url: '',
  })
  ...
  const handleSubmit = async (e) => {
    ...
    setError('')
    try {
      // createBook in api.js maps form fields to Lambda field names via mapBookPayload
      const res = await createBook(form, chapters)
              ...
              <div key={ch.id} className="border border-gray-100 rounded-lg p-4 bg-gray-50">
                ...
                <div className="grid grid-cols-2 gap-3 mb-3">
                  <div>
                    ...
                    <input
                      value={ch.title}
                      onChange={(e) => updateCh(ch.id, 'title', e.target.value)}
                ...
                <div className="grid grid-cols-4 gap-3">
                  <div>
                    <label className="block text-xs text-gray-500 mb-1">
                      <input
                        type="checkbox"
                        checked={ch.is_free}
                        onChange={(e) => updateCh(ch.id, 'is_free', e.target.checked)}
                        className="mr-1"
                      />
                      Free
                    </label>
                    <p className="text-xs text-gray-400">Mark as free chapter</p>
                  </div>
                  <div>
                    <label className="block text-xs text-gray-500 mb-1">Coins price</label>
                    <input
                      type="number"
                      min="0"
                      value={ch.price}
                  ...
                  <div>
                    <label className="block text-xs text-gray-500 mb-1">Ads count</label>
                    <input
                      type="number"
                      min="0"
                      value={ch.ads}
                ...
                {ch.content_url && (
                  <p className="mt-2 text-xs text-green-600">✓ File uploaded</p>
                )}
              </div>
            ))}
          </div>
